...
export function OutreachPitcher({ cvData, language }: OutreachPitcherProps) {
  ...
  const handleGenerate = async () => {
    ...
    setCopied(false);
    try {
      const data = await apiFetchJson<{ success: boolean; pitch?: string; simulated?: boolean; message?: string }>(
        "/api/ai/pitch",
        {
          method: "POST",
          body: JSON.stringify({ cvData, targetCompany, targetRole, jdText, platform, tone, language })
        }
      );
      if (data.success) {
      ...
      if (err instanceof ApiAuthError) {
        alert(isRtl ? "يرجى تسجيل الدخول لاستخدام هذه الميزة." : "Please log in to use this feature.");
      } else {
