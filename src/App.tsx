...
export default function App() {
  ...
  const [cvData, setCvData] = useState<CVData>(() => {
    ...
    if (!saved) return DEFAULT_CV_AR;
    // FIX: كان JSON.parse هنا بدون try/catch — لو الداتا في localStorage تالفة، الصفحة كانت تنكسر بالكامل عند التحميل
    try {
      return JSON.parse(saved);
    } catch {
      console.warn("Corrupted cvData in localStorage, falling back to default.");
      return DEFAULT_CV_AR;
    }
  });
...
${skillsText || (isRtl ? "لا يوجد مهارات مضافة." : "None registered yet.")}
  ...
  const toggleLanguage = () => {
    ...
    if (saved) {
      // FIX: نفس مشكلة JSON.parse غير المحمي — كانت تكسر التطبيق عند تبديل اللغة بداتا تالفة
      try {
        setCvData(JSON.parse(saved));
      } catch {
        console.warn("Corrupted cached CV data, falling back to default.");
  ...
  const executeAiTailoring = async () => {
    ...
    try {
      const data = await apiFetchJson<{ success: boolean; tailoredSummary?: string; tailoredSkills?: string[]; message?: string }>(
        "/api/cv/tailor",
        { method: "POST", body: JSON.stringify({ cvData, jobDescription: tailorJdText }) }
      );
      if (data.success) {
        setCvData((prev) => ({
          ...prev,
        ...
        // FIX: كانت حالة الفشل صامتة تمامًا — المستخدم لا يعرف لماذا لم يتغيّر شيء
        setTailorError(data.message || (isRtl ? "تعذّر تخصيص السيرة الذاتية. حاول مرة أخرى." : "Could not tailor your CV. Please try again."));
      }
    } catch (err) {
      if (err instanceof ApiAuthError) {
        setTailorError(isRtl ? "يرجى تسجيل الدخول لاستخدام هذه الميزة." : "Please log in to use this feature.");
      } else {
        setTailorError((err as Error).message || (isRtl ? "حدث خطأ غير متوقع." : "An unexpected error occurred."));
      }
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-28 md:pb-16 font-sans overflow-x-hidden" dir={isRtl ? "rtl" : "ltr"}>
      ...
      {activeUser && isMobileMenuOpen && (
        <div 
          className="fixed inset-0 z-50 md:hidden bg-slate-900/40 backdrop-blur-xs no-print" 
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  ...
                  <div className="lg:col-span-7 space-y-6">
                    ...
                    <div className="bg-indigo-950 text-white rounded-2xl p-6 border border-indigo-900 space-y-4">
                      ...
                      {tailorError && (
                         <p className="text-rose-400 text-xs font-semibold leading-relaxed font-sans flex items-center gap-1.5">
